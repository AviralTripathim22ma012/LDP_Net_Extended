    ...
    def __init__(self, indim, outdim, stride, dilated_rate):
        ...
        if indim != outdim:
            self.shortcut = nn.Conv2d(
                indim, outdim, kernel_size=1, stride=self.stride, bias=False
            )
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        out = self.trunk(x)
        out = torch.mean(out, dim=(2, 3))  # Average pooling across spatial dimensions
        return out
...
if __name__ == "__main__":
    model = ResNet10()
    x = torch.rand(5, 3, 179, 179)  # Update the shape of the input tensor
    y = model(x)  # Shape: (5, 512)
    print(y.size())
