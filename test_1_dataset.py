...
class EpisodicBatchSampler(object):
    def __init__(self, n_classes, n_way, n_eposide):
        self.n_classes = n_classes
        self.n_way = n_way
        self.n_eposide = n_eposide

    def __len__(self):
        return self.n_eposide

    def __iter__(self):
        for i in range(self.n_eposide):
...
class Eposide_DataManager():
    ...
    def get_data_loader(self, aug):  # parameters that would change on train/val set
        ...
        data_loader_params = dict(batch_sampler=sampler, pin_memory=False, num_workers=0)
